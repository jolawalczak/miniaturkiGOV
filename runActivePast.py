...
xlsx_files = glob.glob("data-active-past/*.xlsx")
...
data_found = False  # Will be set to True if getListActivePast.py returns data at least once
past_date = (datetime.today() - relativedelta(years=3)).strftime('%Y-%m-%d')
csv_file = f"data-active-past/3LATA-AKTYWNE_{past_date}.csv"  # File to be deleted at the end
xlsx_file = f"data-active-past/3LATA-AKTYWNE_{past_date}.xlsx"  # File to be deleted at the end
...
while True:
    ...
    print(f"{current_time} - Running getListActivePast.py with page={page}...")
    result = subprocess.run(["python3", "getListActivePast.py", str(page)],
    ...
    print("Full Output from getListActivePast.py:")

    if result.returncode != 0:
        print(f"Error executing getListActivePast.py on page {page}: {stderr}")
        print(stdout)
        break
    ...
    if "Data saved to" in stdout:
        ...
        print(f"Running convert.py after getListActivePast.py execution (page {page})...")
...
if data_found:
    print("Running collectFirmaData.py after finishing all getListActivePast.py and convert.py executions in real-time:")
...
file_to_send = f"data-active-past/3LATA-AKTYWNE_{past_date}.xlsx"
